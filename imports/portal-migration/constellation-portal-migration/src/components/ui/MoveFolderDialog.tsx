...
import {
  useFolderStore, getDescendantIds,
  Folder, FolderIconType,
...
// ─── Folder icons (matching FolderTree exactly) ───────────────────────────────
...
function DialogBreadcrumb({
  ...
  const path = useMemo<Array<{ id: string | null; name: string }>>(() => {
    ...
    return [{ id: null, name: 'Portal' }, ...segments];
  }, [currentId, folders]);
      ...
      {path.map((seg, i) => {
        ...
        return (
          <Box key={String(seg.id)} sx={{ display: 'flex', alignItems: 'center', gap: '2px' }}>
            {i > 0 && (
              <ChevronRightIcon sx={{ fontSize: 14, color: 'rgba(0,0,0,0.28)', flexShrink: 0 }} />
            )}
            <Typography
              onClick={isLast ? undefined : () => onNavigate(seg.id)}
              sx={{
                fontSize: 13, fontFamily: 'Roboto, sans-serif',
                color: isLast ? '#1f1d25' : BRAND,
...
function FolderRow({
  ...
  const row = (
    <Box
      onClick={disabled ? undefined : () => onSelect(folder.id)}
      onDoubleClick={(!disabled && hasChildren) ? () => onNavigate(folder.id) : undefined}
      sx={{
        display: 'flex', alignItems: 'center', gap: '10px',
        px: '12px', py: '9px', cursor: disabled ? 'not-allowed' : 'pointer',
        borderRadius: '8px', opacity: disabled ? 0.45 : 1,
        bgcolor: selected ? ACTIVE : 'transparent',
        border: `1px solid ${selected ? `${BRAND}30` : 'transparent'}`,
        '&:hover': { bgcolor: disabled ? 'transparent' : selected ? ACTIVE : '#f5f5f7' },
    ...
    >
      ...
      <Typography sx={{
        ...
        color: disabled ? 'rgba(0,0,0,0.3)' : selected ? BRAND : '#1f1d25',
      ...
      {/* Lock icon for disabled (shared) */}
      {disabled && (
        <LockOutlinedIcon sx={{ fontSize: 14, color: 'rgba(0,0,0,0.22)', flexShrink: 0 }} />
      )}
      ...
      {!disabled && hasChildren && (
        <IconButton
          ...
          sx={{
            width: 24, height: 24, p: 0, flexShrink: 0, ml: 'auto',
            color: selected ? BRAND : TEXT_DIM,
            '&:hover': { bgcolor: 'rgba(0,0,0,0.06)', color: BRAND },
          }}
        >
...
// Threshold for "large folder": 3+ total descendants
const LARGE_FOLDER_THRESHOLD = 3;

// Simulated error rate for prototype (~20% of attempts fail)
let _attemptCount = 0;
function shouldSimulateError(): boolean {
  _attemptCount++;
  return _attemptCount % 5 === 0; // every 5th attempt fails
}
...
export function MoveFolderDialog({ open, folderId, folderName, onClose, onSuccess }: MoveFolderDialogProps) {
  ...
  const excludedIds = useMemo(
    () => new Set([folderId, ...getDescendantIds(folderId, folders)]),
    [folderId, folders]
  );
  ...
  const children = useMemo(() => {
    if (isSearching) {
      // Global search across all visible, non-excluded folders
      const q = search.trim().toLowerCase();
      return visibleFolders
        .filter(f => !excludedIds.has(f.id) && f.name.toLowerCase().includes(q))
        .sort((a, b) => a.name.localeCompare(b.name));
    }
    return visibleFolders
      .filter(f => (f.parentId ?? null) === currentBrowseFolderId)
      .sort((a, b) => a.name.localeCompare(b.name));
  }, [visibleFolders, currentBrowseFolderId, search, isSearching, excludedIds]);

  // Navigate into a folder: changes browse level, auto-selects, clears search
  const handleNavigateInto = (id: string) => {
    setCurrentBrowseFolderId(id);
    setSelectedDestId(id);
    setSearch('');
  };

  // Click breadcrumb segment, clear search
  ...
  const handleSelectRow = (id: string) => {
    setSelectedDestId(prev => prev === id ? currentBrowseFolderId : id);
  };
  ...
  // null = root (Portal) is a valid destination; only block self/descendants
  const canMove             = effectiveDestId === null || !excludedIds.has(effectiveDestId);

  // Determine if the folder being moved is "large"
  const isLargeFolder = useMemo(
    () => getDescendantIds(folderId, folders).length >= LARGE_FOLDER_THRESHOLD,
    [folderId, folders]
  );
  ...
  const executeMove = async () => {
    ...
    const delay = isLargeFolder ? 3000 : 1500;
    await new Promise(r => setTimeout(r, delay));

    // Simulate occasional error (every 5th attempt)
  ...
  return (
    ...
    >
      ...
      <DialogTitle sx={{
        ...
        pb: 0.5, fontFamily: 'Roboto, sans-serif', fontWeight: 700, fontSize: 17,
      }}>
        <Box sx={{ flex: 1, pr: 1 }}>
          Move "{folderName}"
        </Box>
        ...
        <>
          <DialogContent dividers sx={{ borderColor: BORDER }}>
            <Box sx={{ py: 4, px: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1.5, textAlign: 'center' }}>
              <ErrorOutlineIcon sx={{ fontSize: 40, color: '#d32f2f', opacity: 0.85 }} />
              <Typography sx={{ fontSize: 15, fontWeight: 600, fontFamily: 'Roboto, sans-serif', color: '#1f1d25' }}>
                Move failed
              </Typography>
              <Typography sx={{ fontSize: 13, fontFamily: 'Roboto, sans-serif', color: TEXT_DIM, maxWidth: 260, lineHeight: 1.6 }}>
                Something went wrong while moving "{folderName}". No changes were made.
              </Typography>
            </Box>
          </DialogContent>
          <DialogActions sx={{ px: 3, py: 2, gap: 1 }}>
            <Button onClick={handleClose} size="small" sx={{ textTransform: 'none', color: TEXT_DIM, borderRadius: '100px', fontFamily: 'Roboto, sans-serif' }}>
              Dismiss
            </Button>
            <Button
              onClick={handleRetry}
              ...
              sx={{ textTransform: 'none', borderRadius: '100px', bgcolor: BRAND, boxShadow: 'none', fontFamily: 'Roboto, sans-serif', fontWeight: 600, '&:hover': { bgcolor: '#3730a3', boxShadow: 'none' } }}
            >
              Try again
            </Button>
          </DialogActions>
        ...
        <>
          ...
          {!isSearching && (
            <Box sx={{ px: 3, pt: 0.5, pb: 1 }}>
              <DialogBreadcrumb
                currentId={currentBrowseFolderId}
          ...
          <Box sx={{ px: 3, pb: 1.25, pt: isSearching ? 1 : 0 }}>
            <Box sx={{
              ...
              px: '10px', height: 34, borderRadius: '8px',
              border: `1px solid ${isSearching ? BRAND : BORDER}`,
              bgcolor: '#fafafa',
              ...
              <InputBase
                ...
                sx={{
                  flex: 1, fontSize: 13, fontFamily: 'Roboto, sans-serif',
                  '& input': { p: 0 },
                  '& input::placeholder': { color: TEXT_DIM, opacity: 1 },
                }}
              />
              {isSearching && (
                <IconButton size="small" onClick={() => setSearch('')} sx={{ p: 0, width: 18, height: 18, color: TEXT_DIM, '&:hover': { color: '#1f1d25' } }}>
            ...
            {/* Search result count */}
            {isSearching && (
              <Typography sx={{ fontSize: 11, color: TEXT_DIM, fontFamily: 'Roboto, sans-serif', mt: '4px', pl: '2px' }}>
                {children.length === 0
                  ? 'No folders found'
                  : `${children.length} folder${children.length !== 1 ? 's' : ''} found`}
              </Typography>
            )}
          </Box>

          {/* ── Large folder progress banner ──────────────────────────────────── */}
          {status === 'loading-large' && (
            <Box sx={{ mx: 3, mb: 1.5, borderRadius: '8px', overflow: 'hidden', border: `1px solid ${BORDER}` }}>
              ...
              <Box sx={{ px: 1.5, py: 1, display: 'flex', gap: 1, alignItems: 'flex-start' }}>
                ...
                <Typography sx={{ fontSize: 12, color: '#473bab99', fontFamily: 'Roboto, sans-serif', lineHeight: 1.5 }}>
          ...
          {showAccountWarning && status === 'idle' && (
            <Box sx={{ mx: 3, mb: 1.5, px: 1.5, py: 1, borderRadius: '8px', bgcolor: WARN_BG, border: `1px solid ${WARN_BORDER}`, display: 'flex', gap: 1 }}>
              ...
              <Typography sx={{ fontSize: 12, color: WARN_TEXT, fontFamily: 'Roboto, sans-serif', lineHeight: 1.5 }}>
                Moving into an account folder will apply that account's visibility restrictions to this folder and all its contents.
              </Typography>
            </Box>
          )}
          ...
          <DialogContent dividers sx={{
            p: '8px', maxHeight: 300, overflowY: 'auto', borderColor: BORDER,
            opacity: isLoading ? 0.4 : 1, pointerEvents: isLoading ? 'none' : 'auto',
            ...
            {children.length === 0 ? (
              <Box sx={{ py: 5, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.5 }}>
                ...
                {isSearching && (
                  <Typography onClick={() => setSearch('')} sx={{ fontSize: 12, color: BRAND, fontFamily: 'Roboto, sans-serif', cursor: 'pointer', '&:hover': { textDecoration: 'underline' } }}>
                    Clear search
                  </Typography>
                )}
              </Box>
              ...
              children.map(folder => {
                ...
                const disabledReason = isShared
                  ? 'Cannot move into a shared folder'
                  : isExcluded
                ...
                return (
                  <FolderRow
                    key={folder.id}
                    folder={folder}
                    selected={selectedDestId === folder.id}
                    disabled={isDisabled}
          ...
          <DialogActions sx={{ px: 3, py: 2, gap: 1 }}>
            ...
            <Button
              ...
              sx={{
                ...
                boxShadow: 'none', fontFamily: 'Roboto, sans-serif', fontWeight: 600, minWidth: 130,
