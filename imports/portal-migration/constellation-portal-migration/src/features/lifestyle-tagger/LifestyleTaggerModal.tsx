// Requires: @mui/material, @mui/icons-material, @emotion/react
...
import { Select, ConfidenceBadge, toConfidenceLevel, Spinner } from '../../components/ui';
import { VerticalTabStrip } from '../../components/ui/VerticalTabStrip';
...
// ─── Re-detect mock results (guaranteed to be in vehicleDatabase) ─────────────
...
// All confidence indicators use the brand primary.
function getConfidenceColor(_score: number): string {
  return tokens.primary;
}

// ─── View-mode tab icons (purple when active) ─────────────────────────────────
const IcoMetadata = () => (
...
// ─── Shared input styling (MetadataPanel standard) ───────────────────────────
const inputSx = {
  '& .MuiOutlinedInput-root': { borderRadius: '6px', fontSize: 13, fontFamily: 'Roboto, sans-serif' },
  '& .MuiOutlinedInput-root.Mui-focused .MuiOutlinedInput-notchedOutline': { borderColor: tokens.primary },
};
...
function VFieldLabel({ children, required }: { children: React.ReactNode; required?: boolean }) {
  return (
    <Typography sx={{ fontSize: 11, fontWeight: 500, color: 'text.secondary', mb: '4px', fontFamily: 'Roboto, sans-serif', display: 'flex', gap: '2px' }}>
...
// ─── Mock detection reasons per field ─────────────────────────────────────────
const FIELD_REASONS = {
...
export function LifestyleTaggerModal({
  ...
  // "Live" suggestion — null until the scan animation completes
  const [liveSuggestion, setLiveSuggestion] = useState<VehicleSuggestion | null>(null);

  // Tracks whether this modal instance is waiting for REAL background analysis
  ...
  // Initialize queue from uploadFiles — runs once per new file set
  useEffect(() => {
    ...
    // Start all analyses in parallel via Promise.allSettled
    ...
    // NOTE: preview URLs are revoked on modal close, not here —
    // React strict mode double-fires effects and would revoke URLs prematurely.
  }, [uploadFiles]);

  // Revoke preview URLs when modal closes or unmounts
  useEffect(() => {
    return () => {
  ...
  // Bulk mode: approve all completed items
  ...
  // Bulk mode: save (call onBulkSave with approved items)
  ...
  // On every asset change: set up scanning based on mode
  useEffect(() => {
    setZoom(1);

    if (!isDetect) {
      // View or Review mode — no scanning at all
      setIsScanning(false);
  ...
  // When real background analysis completes, reveal the result immediately
  useEffect(() => {
  ...
  // Re-detect: clear fields, play scan, then reveal a new result
  ...
  // Bulk-queue computed values (for detect mode queue or multi-select)
    ...
    if (isMulti && isDetect) {
      ...
      return (
        <Box sx={{ flex: 1, display: 'flex', alignItems: 'center', gap: 1.5, minWidth: 0 }}>
          <Typography variant="subtitle2" sx={{ fontWeight: 700, color: tokens.primary, whiteSpace: 'nowrap' }}>
  ...
  // Show thumbnail rail (left) only in detect mode with queue
  const showLeftRail = isDetect && isMulti && queueAssets;
  // Show bottom strip for multi-select in view/review mode
  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, px: 3, py: 1.5, flexShrink: 0 }}>
        {isDetect && <AutoAwesomeIcon sx={{ fontSize: 18, color: tokens.primary }} />}
        ...
        {isDetect && isMulti && pendingCount > 0 && (
          <Chip
            ...
            size="small"
            sx={{ bgcolor: `${tokens.primary}14`, color: tokens.primary, fontWeight: 600, fontSize: 11, height: 22, borderRadius: '100px' }}
          />
        )}

        {isScanning && (
          <Chip
            icon={<CircularProgress size={10} sx={{ color: `${tokens.primary} !important`, ml: '6px !important' }} />}
      ...
      {isBulkMode ? (
        ...
        /* ════ STANDARD LAYOUT (single / detect / review / view) ═════════ */
        <Box sx={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
          ...
          >
            ...
            >
              ...
              {/* Scan overlay — detect mode or re-detect in review mode */}
              {isScanning && <ScanOverlay />}

              {/* Multi-select nav arrows (non-detect mode) */}
              {showBottomStrip && (
                <>
                  <IconButton
                    onClick={handlePrev}
                    sx={{
                      ...
                      bgcolor: 'rgba(255,255,255,0.85)', boxShadow: '0 2px 8px rgba(0,0,0,0.12)',
                      '&:hover': { bgcolor: '#fff' }, width: 36, height: 36,
                    }}
                  >
                    <ChevronLeftIcon />
                  </IconButton>
            ...
            {/* ── Bottom thumbnail strip (multi-select, non-detect) ─────── */}
            {showBottomStrip && (
...
// ─── Metadata Panel (View mode) — tabbed layout ───────────────────────────────
...
function MetadataPanel({ asset, onApproved, onClose }: MetadataPanelProps) {
  ...
  // Reactive: YMMT fields appear only when "Lifestyle" tag is present
    ...
    <Box sx={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
      ...
      <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        <Box sx={{ px: 2.5, py: 1.75, flexShrink: 0 }}>
          <Typography variant="h6" sx={{ fontWeight: 700, fontSize: 18, fontFamily: 'Roboto, sans-serif' }}>Metadata</Typography>
        </Box>
        <Divider />
        ...
        <Box sx={{
          ...
          {activeTab !== 'metadata' ? (
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column' }}>
              ...
              {/* YMMT — only when "Lifestyle" tag is present */}
              {hasLifestyle && (
                <Box sx={{ mb: 2, p: 1.5, borderRadius: '10px', bgcolor: 'rgba(0,0,0,0.03)', border: `1px solid ${ymmtIncomplete ? '#fca5a5' : 'rgba(0,0,0,0.08)'}`, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                  <Typography sx={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.6px', textTransform: 'uppercase', color: 'text.secondary', fontFamily: 'Roboto, sans-serif' }}>
                    Vehicle Info
                  </Typography>
              ...
              <Box sx={{ mb: 2 }}>
                <Button
                  ...
                  sx={{ textTransform: 'none', color: tokens.primary, fontFamily: 'Roboto, sans-serif', fontSize: 13, fontWeight: 500, p: 0, minWidth: 0, '&:hover': { bgcolor: 'transparent', textDecoration: 'underline' } }}
                >
                  New Field
                </Button>
              </Box>
        ...
        <Box sx={{ flexShrink: 0, borderTop: '1px solid', borderColor: 'divider', px: 2.5, py: 1.5, display: 'flex', justifyContent: 'flex-end', gap: 1, bgcolor: 'background.paper' }}>
          {activeTab === 'metadata' ? (
            <Button
              ...
              sx={{ bgcolor: tokens.primary, borderRadius: '100px', textTransform: 'none', boxShadow: 'none', fontWeight: 600, '&:hover': { bgcolor: '#312e81', boxShadow: 'none' }, '&.Mui-disabled': { bgcolor: `${tokens.primary}40`, color: '#fff' } }}
            >
              {saving ? 'Saving…' : 'Save'}
            </Button>
          ) : (
...
function ScanOverlay() {
  return (
    ...
    >
      ...
      {/* Corner brackets */}
      <Box sx={{ position: 'absolute', top: 20, left: 20, width: 22, height: 22, borderTop: `3px solid ${tokens.primary}`, borderLeft: `3px solid ${tokens.primary}`, borderRadius: '2px 0 0 0' }} />
      <Box sx={{ position: 'absolute', top: 20, right: 20, width: 22, height: 22, borderTop: `3px solid ${tokens.primary}`, borderRight: `3px solid ${tokens.primary}`, borderRadius: '0 2px 0 0' }} />
      <Box sx={{ position: 'absolute', bottom: 20, left: 20, width: 22, height: 22, borderBottom: `3px solid ${tokens.primary}`, borderLeft: `3px solid ${tokens.primary}`, borderRadius: '0 0 0 2px' }} />
      <Box sx={{ position: 'absolute', bottom: 20, right: 20, width: 22, height: 22, borderBottom: `3px solid ${tokens.primary}`, borderRight: `3px solid ${tokens.primary}`, borderRadius: '0 0 2px 0' }} />
      ...
      {SCAN_CALLOUTS.map(({ label, top, left, delay }) => (
        <Box
          ...
          sx={{
            ...
            alignItems: 'center',
            gap: 0.75,
            animation: `${calloutFadeIn} 0.55s ease forwards`,
            animationDelay: delay,
            opacity: 0,
          }}
        >
          <Box sx={{ width: 8, height: 8, borderRadius: '50%', bgcolor: tokens.primary, boxShadow: `0 0 0 3px ${tokens.primary}40`, flexShrink: 0 }} />
          <Box sx={{ px: 1, py: 0.375, bgcolor: tokens.primary, borderRadius: '100px', backdropFilter: 'blur(4px)' }}>
      ...
      <Box
        sx={{
          ...
          px: 1.5,
          py: 0.75,
          bgcolor: 'rgba(15,15,30,0.72)',
          backdropFilter: 'blur(8px)',
          borderRadius: '100px',
          border: `1px solid rgba(99,87,225,0.3)`,
        }}
      >
...
// ─── Bottom thumbnail strip (multi-select view/review) ───────────────────────
      ...
      {assets.map(a => {
        ...
        return (
          <Box
            ...
            sx={{
              width: tokens.stripThumbW,
              height: tokens.stripThumbH,
              flexShrink: 0,
              borderRadius: '6px',
              overflow: 'hidden',
...
// ─── Thumbnail rail (left, for detect mode bulk queue) ───────────────────────
...
function ThumbnailRail({ assets, currentId, onJump }: ThumbnailRailProps) {
  ...
  return (
    <Box sx={{ width: tokens.railWidth, flexShrink: 0, display: 'flex', flexDirection: 'column', borderRight: '1px solid', borderColor: 'divider', bgcolor: '#F7F8FA' }}>
      {/* Rail header */}
      <Box sx={{ px: 1.5, pt: 1.5, pb: 1, flexShrink: 0 }}>
          ...
          {assets.map((a) => (
            <Box
              ...
              sx={{
                ...
                bgcolor: a.aiStatus === 'approved' ? '#3730a3' : a.aiStatus === 'not-vehicle' ? '#6B7280' : a.id === currentId ? tokens.primary : `${tokens.primary}30`,
        ...
        <Box sx={{ display: 'flex', gap: 1, mt: 0.75 }}>
          {approvedCount > 0  && <Typography variant="caption" sx={{ fontSize: 10, color: '#3730a3', fontWeight: 600 }}>{approvedCount} ✓</Typography>}
          ...
          return (
            <Box
              ...
              sx={{
                ...
                bgcolor: isCurrent ? `${tokens.primary}0F` : 'transparent',
                borderLeft: isCurrent ? `3px solid ${tokens.primary}` : '3px solid transparent',
                transition: 'background-color 0.12s',
                '&:hover': isPending && !isCurrent ? { bgcolor: `${tokens.primary}0A` } : {},
              }}
            >
              <Box sx={{ position: 'relative', borderRadius: '6px', overflow: 'hidden', mb: 0.625, border: isCurrent ? `1.5px solid ${tokens.primary}` : '1.5px solid transparent' }}>
                ...
                {!isApproved && !isDismissed && (
                  <Box sx={{ position: 'absolute', top: 4, left: 4, width: 18, height: 18, borderRadius: '50%', bgcolor: isCurrent ? tokens.primary : 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <Typography variant="caption" noWrap sx={{ display: 'block', fontSize: 10.5, lineHeight: 1.3, color: isCurrent ? tokens.primary : isApproved || isDismissed ? 'text.disabled' : 'text.secondary', fontWeight: isCurrent ? 600 : 400 }}>
                {a.name}
              </Typography>
            </Box>
          );
...
// ─── Bulk thumbnail rail (left, for bulk upload) ────────────────────────────
        ...
        return (
          <Box
            ...
            sx={{
              ...
              overflow: 'hidden',
              cursor: 'pointer',
              bgcolor: isActive ? `${tokens.primary}1A` : 'transparent',
          ...
          >
            ...
            {item.status === 'analyzing' && (
              <Box sx={{
                ...
                <CircularProgress size={12} sx={{ color: tokens.primary }} />
              </Box>
            )}
            {item.status === 'approved' && (
              <Box sx={{
                ...
                <CheckCircleIcon sx={{ fontSize: 14, color: tokens.primary }} />
              </Box>
            )}
...
// ─── Bulk metadata panel (right, for bulk upload) ───────────────────────────
...
function BulkMetadataPanel({
  ...
  // Form fields populated from analysis result
  ...
  // Sync form fields when active item or its result changes
  useEffect(() => {
    if (!result) {
  ...
  const makeBadge = (field: 'year' | 'make' | 'model' | 'trim') => {
    ...
    return (
      <ConfidenceBadge level={toConfidenceLevel(conf)} score={conf} />
    );
  };
  ...
  return (
    <Box sx={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
      ...
      {/* ── Vertical tab strip (Metadata / Enhance / Download) ──────── */}
      ...
      <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        <Box sx={{ px: 2.5, py: 1.75, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Typography variant="h6" sx={{ fontWeight: 700, fontSize: 18, fontFamily: 'Roboto, sans-serif' }}>
            Metadata
          </Typography>
        ...
        <Box sx={{
          ...
          {activeTab !== 'metadata' ? (
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
              ...
              >
                <AutoAwesomeIcon sx={{ fontSize: 16, color: tokens.primary }} />
                <Typography sx={{ fontSize: 13, fontWeight: 600, color: tokens.primary, lineHeight: 1.3 }}>
                  Auto-detecting vehicle YMMT information
                </Typography>
              </Box>

              {/* Spinner + detecting label */}
            ...
            /* ── Completed: editable YMMT fields ───────────────────────── */
        ...
        {/* ── Footer: Approve & save / Approve all & save ─────────────── */}
        <Box
          sx={{
        ...
        >
          <Button
            ...
            sx={{
              bgcolor: tokens.primary, borderRadius: '100px', textTransform: 'none',
              boxShadow: 'none', fontWeight: 600, whiteSpace: 'nowrap',
              '&:hover': { bgcolor: tokens.primaryHover, boxShadow: 'none' },
              '&.Mui-disabled': { bgcolor: `${tokens.primary}40`, color: '#fff' },
            }}
          >
            Approve &amp; save
          </Button>
          ...
          <Button
            ...
            onClick={handleApproveAllAndSave}
...
function ReviewPanel({
  ...
  // Which fields have been revealed (for detect mode stagger)
  ...
  // On suggestion change: populate fields
  useEffect(() => {
    setSaving(false);

    if (mode === 'review') {
      if (isScanning) {
        // Re-detect in progress — clear fields like detect mode
        setYear(''); setMake(''); setModel(''); setTrim(''); setTags([]);
        setYearRevealed(false); setMakeRevealed(false);
        setModelRevealed(false); setTrimRevealed(false);
        return;
      }

      if (suggestion && !yearRevealed) {
        // Re-detect just finished — stagger-reveal the new results
        const ss = suggestion;
        setTags(ss.tags ?? []);
      ...
      // Review mode: pre-populate from asset data immediately
      const src = suggestion ?? {
        year: asset.year ?? '', make: asset.make ?? '', model: asset.model ?? '', trim: asset.trim ?? '',
      };
      ...
      setYearRevealed(true); setMakeRevealed(true);
      setModelRevealed(true); setTrimRevealed(true);
      return;
    }

    // Detect mode: reset then stagger-fill
    setYear(''); setMake(''); setModel(''); setTrim(''); setTags([]);
    setYearRevealed(false); setMakeRevealed(false);
    setModelRevealed(false); setTrimRevealed(false);
  ...
  const makeBadgeNode = (field: 'year' | 'make' | 'model' | 'trim', revealed: boolean, confidence: number) => {
    ...
    return (
      <ConfidenceBadge level={toConfidenceLevel(confidence)} score={confidence} tooltipContent={tooltipContent} />
    );
  };
  ...
  const overallColor = isScanning ? tokens.primary : getConfidenceColor(s?.confidence ?? 0);
  ...
  return (
    <Box sx={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
      ...
      {/* ── Overall confidence + Re-detect ──────────────────────────── */}
      {(isDetect || s) && (
        <Box
          sx={{
            px: 2.5, pt: 2, pb: 1.75,
            bgcolor: `${tokens.primary}07`,
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', mb: 1.25 }}>
            <Box>
              {isScanning ? (
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25 }}>
                  <CircularProgress size={22} sx={{ color: tokens.primary }} />
                  <Box>
                    <Typography variant="body2" sx={{ fontWeight: 700, color: tokens.primary, lineHeight: 1.2 }}>
                      Scanning…
                    </Typography>
                ...
                <Box>
                  <Typography
                    variant="h4"
                    sx={{ fontWeight: 800, color: overallColor, lineHeight: 1, fontSize: 32 }}
                  >
                    {overallPct}
            ...
            <Button
              ...
              sx={{
                textTransform: 'none', borderRadius: '100px', fontSize: 12, fontWeight: 600,
                color: tokens.primary, borderColor: `${tokens.primary}50`,
                '&:hover': { borderColor: tokens.primary, bgcolor: `${tokens.primary}0A` },
          ...
          <LinearProgress
            ...
            sx={{
              ...
              bgcolor: isScanning ? `${tokens.primary}20` : `${overallColor}20`,
              '& .MuiLinearProgress-bar': {
                bgcolor: isScanning ? tokens.primary : overallColor,
                borderRadius: '3px',
      ...
      <Box sx={{ flex: 1, overflowY: 'auto', px: 2.5, py: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        {/* ── File info — only shown after scan completes ──────────── */}
        {!isScanning && (
          <>
            <Divider />
            <Box>
              ...
              {[
                ...
                <Box
                  ...
                  sx={{
                    ...
                    py: 0.875, borderBottom: '1px solid', borderColor: 'divider',
                    '&:last-child': { borderBottom: 'none' },
                  }}
                >
                  <Typography variant="caption" color="text.secondary" sx={{ fontSize: 12 }}>{label}</Typography>
                  <Typography variant="caption" sx={{ fontWeight: 500, fontSize: 12, color: 'text.primary', maxWidth: 160, textAlign: 'right', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {value}
                  </Typography>
                </Box>
              ))}
      ...
      )}{/* end reviewTab === 'metadata' */}

      {/* ── Correction notice ── */}
      {hasCorrected && !isScanning && (
        <Box
          sx={{
            flexShrink: 0, px: 2.5, py: 0.875,
            bgcolor: `${tokens.primary}0A`, borderTop: `1px solid ${tokens.primary}22`,
            display: 'flex', alignItems: 'center', gap: 0.75,
          }}
        >
          <CheckCircleOutlineIcon sx={{ fontSize: 14, color: '#22C55E' }} />
          <Typography variant="caption" sx={{ fontSize: 11, color: tokens.primary, lineHeight: 1.4 }}>
      ...
      <Box
        sx={{
          flexShrink: 0, borderTop: '1px solid', borderColor: 'divider',
          px: 2.5, py: 1.75, display: 'flex', alignItems: 'center', gap: 1, bgcolor: 'background.paper',
        }}
      >
        <Button
          size="small"
        ...
        {onSkip && (
          <Tooltip title="Save for later and review next">
            <Button
              ...
              sx={{ textTransform: 'none', color: 'text.secondary', fontWeight: 500, '&:hover': { bgcolor: 'action.hover' } }}
            >
              Skip
            </Button>
          </Tooltip>
