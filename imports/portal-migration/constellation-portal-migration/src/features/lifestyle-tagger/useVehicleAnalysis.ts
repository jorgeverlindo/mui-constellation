...
// ─── Make → lifestyle/tags defaults ──────────────────────────────────────────
// The server returns make + confidence. We fill in lifestyle/tags/model/trim/year
// with plausible defaults until a full YMMT endpoint is available.
...
// Can be called per-item independently (e.g. from Promise.allSettled in a bulk
// upload queue) without requiring a React hook or shared state.
...
export async function analyzeVehicle(file: File): Promise<VehicleAnalysisResult> {
  ...
  const defaults = (make && MAKE_DEFAULTS[make]) ?? FALLBACK_DEFAULTS;
...
export function useVehicleAnalysis(): UseVehicleAnalysisReturn {
  ...
  const analyze = useCallback(async (file: File) => {
    ...
    try {
      const analysisResult = await analyzeVehicle(file);
