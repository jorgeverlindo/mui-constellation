...
// ─── Snackbar stub ────────────────────────────────────────────────────────────
// Replaces shadcn/sonner toast() from vw-funds-2.
// Fires a CustomEvent so a MUI Snackbar host (mounted once near the root)
// can listen and render the notification without coupling this context to MUI.
...
export function emitSnackbar(message: string, severity: SnackbarMsg['severity'] = 'success') {
  const detail: SnackbarMsg = { message, severity };
  window.dispatchEvent(new CustomEvent('constellation:snackbar', { detail }));
}
