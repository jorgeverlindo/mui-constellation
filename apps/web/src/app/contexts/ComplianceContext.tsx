/**
 * ComplianceContext — shared state for the Compliance / Web Monitoring feature.
 *
...
const DELETED_INFRACTIONS_STORAGE_KEY = 'vw-funds-2:deletedInfractionIds'; // right-click delete
...
// dealer identity per role — drives Compliance scope, AI auto-fill, and "submitted by" labels
...
// kept for backward compatibility — used as default when userType isn't a known dealer role
...
export function ComplianceProvider({ children }: { children: ReactNode }) {
  // user-added infractions (OEM Add Infraction + dealer Report Infraction flows)
  const [userAddedInfractions, setUserAddedInfractions] = useLocalStorage<WCMItem[]>(
    ...
    (raw) => {
      ...
      // migrate older entries that pre-date the `source` field
      return parsed.map(item => item.source ? item : { ...item, source: 'Manually Added' as const });
    },
  );

  // right-click delete — deleted IDs (covers both static WCM_DATA and userAddedInfractions)
  ...
  // dealer-side "seen" infraction IDs — drives the unread bell badge for the dealer
  ...
  // update status of a user-added (dealer-reported or OEM-added) infraction
  ...
  // Infractions targeted at a given dealership — these surface as bell notifications.
