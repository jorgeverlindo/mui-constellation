...
// adaptiveColor — browser-safe stubs (server implementation not needed for rendering)
function sampleImageUrl(_url: string, _opts?: object): Promise<string> { return Promise.resolve(_url); }
function deriveAdaptiveColors(_url: string): { primary: string; dominant: string; contrast: string } {
  void _url;
  return { primary: '#000000', dominant: '#000000', contrast: '#ffffff' };
}
import { zoneConfigs, isKeyMessageTextLayout, isPharmaZoneConfig } from "../lib/template-zone-configs";
import type { TemplateZoneConfig, SingleProductTextLayout, MultiProductTextLayout, KeyMessageTextLayout } from "../lib/template-zone-configs";
import { useProjectStoreSafe } from "../lib/project-store";
import { brandKits } from "../lib/mock-data";
import type { BackgroundCollection } from "../logos-backgrounds/BackgroundCollectionCard";
import type { PharmaZoneConfig } from "../lib/template-zone-configs";
    ...
    return (
      <div style={containerStyle}>
        <img
          ...
          style={{ objectFit: "contain", objectPosition: "center bottom", position: "absolute", inset: 0, width: "100%", height: "100%" }}
        />
      </div>
    );
  }
  ...
  return (
    <div style={{ width: size, height: size, position: "relative", display: "block" }}>
      {forExport
        ...
        : <img src={primaryLogoSrc} alt="Primary Logo" style={{ objectFit: "contain", position: "absolute", inset: 0, width: "100%", height: "100%" }} />}
    </div>
  );
}
  ...
  return (
    <div style={{ width: size, height: size, position: "relative", display: "block" }}>
      {forExport
        ...
        : <img src={eventLogoSrc} alt="Event Logo" style={{ objectFit: "contain", position: "absolute", inset: 0, width: "100%", height: "100%" }} />}
    </div>
  );
}
    ...
    return (
      <div style={{ position: "absolute", inset: 0, width, height }}>
        {forExport
          ...
          : <img src={image} alt="" sizes={`${width}px`} style={{ objectFit: "cover", position: "absolute", inset: 0, width: "100%", height: "100%" }} />}
      </div>
    );
  }
  return (
