  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: '16px' } }}>
      ...
      <DialogContent sx={{ pt: '8px !important' }}>
        <TextField
          autoFocus fullWidth size="small" value={name}
          onChange={e => setName(e.target.value)}
          onKeyDown={e => { if (e.key === 'Enter') handleSave(); if (e.key === 'Escape') onClose(); }}
          sx={{ '& .MuiOutlinedInput-root': { borderRadius: '8px', fontSize: 14, '&.Mui-focused fieldset': { borderColor: BRAND } } }}
        />
      </DialogContent>

      <DialogActions sx={{ px: 3, py: 2, gap: 1 }}>
        <Button onClick={onClose} size="small" sx={{ textTransform: 'none', color: TEXT_DIM, borderRadius: '100px' }}>
          Cancel
        </Button>
        <Button onClick={handleSave} variant="contained" size="small"
          disabled={!name.trim() || name.trim() === currentName}
          sx={{ textTransform: 'none', borderRadius: '100px', bgcolor: BRAND, boxShadow: 'none', fontWeight: 600, '&:hover': { bgcolor: brand.accentHover, boxShadow: 'none' }, '&.Mui-disabled': { bgcolor: `${BRAND}40`, color: '#fff' } }}>
          Rename
        </Button>
      </DialogActions>
    </Dialog>
