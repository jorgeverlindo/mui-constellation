...
function JobRow({ job }: { job: UploadJob }) {
  return (
    <Box sx={{
      ...
      <Box sx={{ width: 36, height: 36, borderRadius: '6px', flexShrink: 0, bgcolor: surfaceTokens.canvas, overflow: 'hidden' }}>
        {job.previewUrl && (
          <Box component="img" src={job.previewUrl} alt={job.name} sx={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
        )}
      </Box>
      <Typography sx={{ flex: 1, fontSize: 13, color: ink.primary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        {job.name}
      </Typography>
...
function FolderJobCard({
  ...
  const StatusIcon = () => {
    if (status === 'uploading') return <CircularProgress size={18} thickness={3} sx={{ color: BRAND, flexShrink: 0 }} />;
    if (status === 'done')      return <CheckCircleIcon sx={{ fontSize: 20, color: '#22c55e', flexShrink: 0 }} />;
    if (status === 'partial')   return <WarningAmberIcon sx={{ fontSize: 20, color: '#f59e0b', flexShrink: 0 }} />;
    if (status === 'empty')     return <FolderOutlinedIcon sx={{ fontSize: 20, color: TEXT_DIM, flexShrink: 0 }} />;
    if (status === 'error')     return <ErrorOutlineIcon sx={{ fontSize: 20, color: '#ef4444', flexShrink: 0 }} />;
    return null;
  };
  ...
  const summary = () => {
    ...
    if (status === 'done')    return `${uploadedCount} asset${uploadedCount !== 1 ? 's' : ''} uploaded`;
    if (status === 'partial') return `${uploadedCount} uploaded · ${skippedCount} skipped`;
    if (status === 'empty')   return 'Folder created — no supported assets found';
    if (status === 'error')   return 'Failed to upload folder';
    return '';
  };
    ...
    <Box sx={{ borderBottom: `1px solid ${DIVIDER}` }}>
      <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: '10px', px: '16px', py: '12px' }}>
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Typography sx={{ fontSize: 13, fontWeight: 600, color: ink.primary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
        ...
        <Box sx={{ bgcolor: '#ffffff', borderTop: `1px solid ${DIVIDER}` }}>
          {fileJobs.map(job => (
            <Box key={job.id} sx={{ display: 'flex', alignItems: 'center', gap: '10px', px: '20px', py: '7px', borderBottom: `1px solid ${DIVIDER}`, '&:last-child': { borderBottom: 'none' } }}>
              <Box sx={{ width: 28, height: 28, borderRadius: '4px', bgcolor: surfaceTokens.canvas, overflow: 'hidden', flexShrink: 0 }}>
                {job.previewUrl && <Box component="img" src={job.previewUrl} sx={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />}
              </Box>
              <Typography sx={{ flex: 1, fontSize: 12, color: ink.primary, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {job.name}
              </Typography>
      ...
      {(status === 'partial' || (status === 'done' && skippedCount > 0)) && skippedCount > 0 && (
        <>
          ...
          >
            ...
            <Typography sx={{ flex: 1, fontSize: 12, color: '#92400e' }}>
          ...
          <Collapse in={skippedOpen}>
            <Box sx={{ bgcolor: '#fffbf0', borderTop: `1px solid ${DIVIDER}` }}>
              {folderJob.skippedFileNames.map(name => (
                <Typography key={name} sx={{ fontSize: 11, color: '#92400e', px: '24px', py: '5px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {name}
                </Typography>
              ))}
            </Box>
      ...
      {status === 'empty' && (
        <Box sx={{ px: '16px', py: '8px', bgcolor: '#ffffff', borderTop: `1px solid ${DIVIDER}` }}>
          <Typography sx={{ fontSize: 11, color: TEXT_DIM, fontStyle: 'italic' }}>
      ...
      {status !== 'uploading' && status !== 'error' && (
        <Box sx={{ display: 'flex', gap: '20px', px: '16px', py: '10px', borderTop: `1px solid ${DIVIDER}`, bgcolor: '#ffffff' }}>
          <Typography
            ...
            sx={{ fontSize: 13, fontWeight: 600, color: BRAND, cursor: 'pointer', '&:hover': { textDecoration: 'underline' } }}
          >
            Open Folder
          </Typography>
...
function buildTitle(folderJobs: FolderUploadJob[], standaloneUploading: number, standaloneDone: number): string {
...
export function UploadActivityMonitor() {
  ...
  const { navigateToFolder } = usePortalNav();
        ...
        <>
          <Box sx={{ maxHeight: 360, overflowY: 'auto', '&::-webkit-scrollbar': { width: 4 }, '&::-webkit-scrollbar-thumb': { bgcolor: 'rgba(0,0,0,0.15)', borderRadius: 2 } }}>
            {folderJobs.map(fj => (
              <FolderJobCard
                ...
                onOpenFolder={() => navigateToFolder(fj.createdFolderId)}
          ...
          {allDone && (standaloneJobs.length > 0 || completedAssets.length > 0) && (
            <Box sx={{ display: 'flex', gap: '20px', px: '16px', py: '12px', borderTop: `1px solid ${DIVIDER}`, bgcolor: '#ffffff' }}>
              <Typography sx={{ fontSize: 13, fontWeight: 600, color: BRAND, cursor: 'pointer', '&:hover': { textDecoration: 'underline' } }}>
                View In Folder
              </Typography>
