...
/** Sanitize HTML from contentEditable before storing or rendering.
 * DOMPurify is not installed — uses a simple DOM-based sanitizer as fallback.
 */
export function sanitizeHtml(html: string): string {
  try {
    const div = document.createElement("div");
    div.innerHTML = html;
    // Walk the tree and remove disallowed nodes
    const walker = document.createTreeWalker(div, NodeFilter.SHOW_ELEMENT);
    const toRemove: Element[] = [];
    let node = walker.currentNode as Element;
    while (node) {
      if (node !== div && node.nodeType === Node.ELEMENT_NODE) {
