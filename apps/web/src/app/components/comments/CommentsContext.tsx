...
// Pure React — no UI framework dependencies.
// Ported directly from vw-funds-2 (no changes needed except path aliases).
import React, {
...
import type {
  CommentData, CommentsContextValue, NotifAction as _NotifAction,
  NotifItem, Reply,
} from "./types";
import {
  COMMENT_USERS, COMMENTS_STORAGE_KEY, CURRENT_USER,
  NOTIFS_STORAGE_KEY, getUserById,
} from "./constants";
import {
  extractMentionIds, genId, htmlToPlainText,
...
function commentsReducer(state: CommentData[], action: CommentAction): CommentData[] {
  switch (action.type) {
    ...
    case "EDIT":   return state.map(c => c.id === action.commentId ? { ...c, message: action.message, isEdited: true } : c);
    case "DELETE": return state.filter(c => c.id !== action.commentId);
    case "PIN":    return state.map(c => c.id === action.commentId ? { ...c, isPinned: action.pinned, pinnedAt: action.pinned ? action.pinnedAt : undefined } : c);
    case "ADD_REPLY":    return state.map(c => c.id === action.commentId ? { ...c, replies: [...c.replies, action.reply] } : c);
    case "EDIT_REPLY":   return state.map(c => c.id === action.commentId ? { ...c, replies: c.replies.map(r => r.id === action.replyId ? { ...r, message: action.message, isEdited: true } : r) } : c);
    case "DELETE_REPLY": return state.map(c => c.id === action.commentId ? { ...c, replies: c.replies.filter(r => r.id !== action.replyId) } : c);
    default: return state;
  }
}

type NotifDispatchAction =
  | { type: "SET";      items: NotifItem[] }
  | { type: "ADD";      item:  NotifItem }
  | { type: "MARK";     id:    string }
  | { type: "MARK_ALL" };

function notifsReducer(state: NotifItem[], action: NotifDispatchAction): NotifItem[] {
  switch (action.type) {
...
export function CommentsProvider({ contextId, contextName = "", currentUserId, children }: CommentsProviderProps) {
  ...
  const persistComments = useCallback((next: CommentData[]) => { saveToStorage(storageKey, next); }, [storageKey]);
  const persistNotifs = useCallback((next: NotifItem[]) => { saveToStorage(NOTIFS_STORAGE_KEY, next); }, []);

  const emitMentionNotifs = useCallback((
    message: string, action: _NotifAction, commentId: string, replyId?: string, entityId?: string,
  ) => {
    ...
    mentionIds.forEach(recipientId => {
      ...
      const notif: NotifItem = {
        id: genId("n"), action, actorId: currentUser.id, recipientId,
        projectId: contextId, projectName: contextName,
        timestamp: Date.now(), isRead: false,
        targetCommentId: commentId, targetReplyId: replyId, targetEntityId: entityId, preview,
      };
      dispatchNotifs({ type: "ADD", item: notif });
      persistNotifs([notif, ...notifsRef.current]);
    });
  }, [currentUser, contextId, contextName, persistNotifs]);

  const addComment = useCallback((draft: Omit<CommentData, "id" | "timestamp" | "authorId">) => {
    const comment: CommentData = { ...draft, id: genId("c"), projectId: contextId, authorId: currentUser.id, timestamp: Date.now() };
    dispatchComments({ type: "ADD", comment });
    persistComments([comment, ...commentsRef.current]);
  ...
  const editComment = useCallback((commentId: string, message: string) => {
    ...
    persistComments(commentsRef.current.map(c => c.id === commentId ? { ...c, message, isEdited: true } : c));
  }, [persistComments]);

  const deleteComment = useCallback((commentId: string) => {
    dispatchComments({ type: "DELETE", commentId });
    persistComments(commentsRef.current.filter(c => c.id !== commentId));
  }, [persistComments]);
  ...
  const pinComment = useCallback((commentId: string, pinned: boolean) => {
    ...
    persistComments(commentsRef.current.map(c => c.id === commentId ? { ...c, isPinned: pinned, pinnedAt } : c));
  }, [persistComments]);

  const addReply = useCallback((commentId: string, draft: Omit<Reply, "id" | "timestamp" | "authorId">) => {
    const reply: Reply = { ...draft, id: genId("r"), authorId: currentUser.id, timestamp: Date.now() };
    dispatchComments({ type: "ADD_REPLY", commentId, reply });
    persistComments(commentsRef.current.map(c => c.id === commentId ? { ...c, replies: [...c.replies, reply] } : c));
  ...
  const editReply = useCallback((commentId: string, replyId: string, message: string) => {
    ...
    persistComments(commentsRef.current.map(c => c.id === commentId ? { ...c, replies: c.replies.map(r => r.id === replyId ? { ...r, message, isEdited: true } : r) } : c));
  }, [persistComments]);

  const deleteReply = useCallback((commentId: string, replyId: string) => {
    dispatchComments({ type: "DELETE_REPLY", commentId, replyId });
    persistComments(commentsRef.current.map(c => c.id === commentId ? { ...c, replies: c.replies.filter(r => r.id !== replyId) } : c));
  }, [persistComments]);

  const markRead = useCallback((notifId: string) => {
    dispatchNotifs({ type: "MARK", id: notifId });
    persistNotifs(notifsRef.current.map(n => n.id === notifId ? { ...n, isRead: true } : n));
  }, [persistNotifs]);

  const markAllRead = useCallback(() => {
    dispatchNotifs({ type: "MARK_ALL" });
    persistNotifs(notifsRef.current.map(n => ({ ...n, isRead: true })));
  }, [persistNotifs]);
  ...
  const openPanelForEntity = useCallback((entity: import("./types").EntityRef) => { setPendingEntity(entity); setIsPanelOpen(true); setIsNotifOpen(false); }, []);
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(new CustomEvent('comment-notifs-changed', { detail: { notifs: myNotifs, unreadCount: unread } }));
    return () => { window.dispatchEvent(new CustomEvent('comment-notifs-changed', { detail: { notifs: [], unreadCount: 0 } })); };
  }, [notifications, currentUser]);

  useEffect(() => {
    const handler = (e: Event) => { const { id } = (e as CustomEvent).detail as { id: string }; markRead(id); };
  ...
  useEffect(() => { if (isPanelOpen) window.dispatchEvent(new CustomEvent("comments-opened")); }, [isPanelOpen]);
  ...
  useEffect(() => {
    ...
    setIsPanelOpen(false); setIsNotifOpen(false); setPendingEntity(null);
